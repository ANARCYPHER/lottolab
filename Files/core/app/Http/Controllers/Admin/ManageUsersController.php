...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $totalWin = $user->wins->sum('win_bonus');
        $reff = User::where('id',$user->ref_by)->first();

        return view('admin.users.detail', compact('pageTitle', 'user','totalDeposit','totalWithdraw','totalTransaction','countries', 'totalWin', 'reff'));
    }
    ...
    public function wins($id)
    {
        ...
        $winners = $user->wins()->latest()->with('tickets','lotteries','tickets.user')->paginate(getPaginate());
    ...
    public function tickets($id)
    {
        ...
        $tickets = $user->lotteryTickets()->latest()->with('lottery','phase','user')->paginate(getPaginate());
        $empty_message = 'No tickets found';
        return view('admin.users.tickets', compact('pageTitle', 'user', 'tickets', 'empty_message'));
    }

    public function ticketsSearch(Request $request,$id)
    {
        $user = User::findOrFail($id);
        $search = $request->search;
        $tickets = Ticket::where('user_id',$user->id)->where('ticket_number', 'like', "%$search%")->with('user','lottery','phase')->paginate(getPaginate());
        $pageTitle = 'User Tickets : ' . $user->username;
        $empty_message = 'No tickets found';
        return view('admin.users.tickets', compact('pageTitle', 'user', 'tickets', 'empty_message','search'));
    }
    ...
    public function referralCommissionsDeposit(Request $request, $id)
    {
        ...
        $logs = $user->commissions()->where('commission_type','deposit')->with('user','bywho')->paginate(getPaginate());
        $empty_message = 'No Data Found!';
        return view('admin.users.commission-log', compact('pageTitle', 'user', 'logs', 'empty_message'));
    }
    ...
    public function referralCommissionsBuy(Request $request, $id)
    {
        $user = User::findOrFail($id);
        $pageTitle = 'User Commission Log: ' . $user->username;
        $logs = $user->commissions()->where('commission_type','buy')->with('user','bywho')->paginate(getPaginate());
        $empty_message = 'No Data Found!';
        return view('admin.users.commission-log', compact('pageTitle', 'user', 'logs', 'empty_message'));
    }
    ...
    public function referralCommissionsWin(Request $request, $id)
    {
        $user = User::findOrFail($id);
        $pageTitle = 'User Commission Log: ' . $user->username;
        $logs = $user->commissions()->where('commission_type','win')->with('user','bywho')->paginate(getPaginate());
        $empty_message = 'No Data Found!';
        return view('admin.users.commission-log', compact('pageTitle', 'user', 'logs', 'empty_message'));
    }
}
